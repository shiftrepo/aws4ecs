...
import logging
import tenacity
import tempfile # tempfile モジュールを追加
...
from langchain_community.document_loaders import PyPDFLoader, WebBaseLoader, TextLoader # TextLoader を追加
...
def load_documents(source_type, source_path):
    ...
    if source_type == "wiki":
        ...
        pdf_documents = loader.load() # PDFをロード (ページごとに分割された Document オブジェクトのリスト)
        full_text = ""
        for doc in pdf_documents: # 各ページのテキストを結合
            full_text += doc.page_content + "\n\n" # ページ区切りとして改行を挿入
        

        # 一つのテキストファイルに保存 (一時ファイルを使用)
...
if __name__ == "__main__":
    ...
    for document in documents:
        ...
        print(f"ドキュメント挿入中: {source_info}") # どのドキュメントを挿入しているか表示
